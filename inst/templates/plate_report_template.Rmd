...
Plate was run on: `r if(!is.null(plate$plate_datetime)) format(plate$plate_datetime, "%d-%m-%Y %H:%M:%S") else "Date of running plate was not found in Luminex file"`.  
...
for (i in seq_along(plate$analyte_names)) {
  ...
  cat(paste0('<div class="plot-container">',
             ...
             '" aria-expanded="false">'))
  # Generate the plot
  model <- create_standard_curve_model_analyte(plate, plate$analyte_names[i])
...
for (i in seq_along(plate$analyte_names)) {
  ...
  # Plot MFI
  print(plot_mfi_for_analyte(plate,
                             plate$analyte_names[i], 
                             plot_type = "violin"))

  # Plot Standard Curve
  print(plot_standard_curve_analyte(plate, plate$analyte_names[i]))
  if (params$use_model) {
    model <- models_list[[plate$analyte_names[i]]]
    print(plot_standard_curve_analyte_with_model(plate, model))
  }
