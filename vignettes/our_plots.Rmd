...
There is few more useful parameters in this function. Mainly `legend_type` By default it is set to `NULL` which means that the legend type
is determined based on the `monochromatic` value. If `monochromatic` is equal to `TRUE` then legend type is set to `date`, if it is `FALSE` then legend type is set to `plate_name`. User can override this behavior by setting explicitly `legend_type` to `date` or `plate_name`.
...
There might occur a situation, where the plate names are really long, and simply do not fit in the legend box. For such situations, we have prepared the `max_legend_items_per_row` parameter, which automatically decides how many legend elements should be placed in one row of the legend. By default it is set to `3`.


```{r}
plot_standard_curve_stacked(list_of_plates, "Adenovirus.T3", monochromatic = FALSE, legend_position="top", max_legend_items_per_row = 2)
```
